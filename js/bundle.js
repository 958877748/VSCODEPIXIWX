...
(function (主空间) {
    class 地图类 extends PIXI.Container {
        constructor(地图图集, 地图数据) {
            ...
            this.计算实际要显示的块多少();
            let 数据 = this.地图数据.所有地图层[0];
            //根据显示矩形去显示所有块
            //思路1:算出矩形4个顶点,根据4顶点算出4个地图块
            //其余地图块便是他们中间的,补上即可
            //顶点顺序为从左上角开始顺时针排列
            let 左上角顶点 = { x: this.显示矩形.x, y: this.显示矩形.y };
            // let 顶点2 = { x: this.显示矩形.x + this.显示矩形.width, y: this.显示矩形.y }
            // let 顶点3 = { x: this.显示矩形.x + this.显示矩形.width, y: this.显示矩形.y + this.显示矩形.height }
            // let 顶点4 = { x: this.显示矩形.x, y: this.显示矩形.y + this.显示矩形.height }
            //根据顶点1计算地图块区域
            ...
            //根据地图块坐标显示这4块
            // this.根据地图块坐标显示地图块({ x: 地图块区域.x, y: 地图块区域.y })
            // this.根据地图块坐标显示地图块({ x: 地图块区域.x + 地图块区域.width, y: 地图块区域.y })
            // this.根据地图块坐标显示地图块({ x: 地图块区域.x + 地图块区域.width, y: 地图块区域.y + 地图块区域.height })
            // this.根据地图块坐标显示地图块({ x: 地图块区域.x, y: 地图块区域.y + 地图块区域.height })
        }
        根据地图块坐标显示地图块(坐标) {
            let 地图块数据 = this.根据地图块坐标取地图块数据(坐标);
            //很据地图块数据从池中拿出地图块对象
            let 地图块 = this.地图块池.获取地图块(地图块数据);
            //将地图块添加到地图上
            地图块.x = 坐标.x * 24;
            地图块.y = 坐标.y * 24;
            this.addChild(地图块);
